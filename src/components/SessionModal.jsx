...
export function SessionModal({ week, onClose, onUpdate }) {
  ...
  const handlePickerSelect = (item) => {
    ...
    if (picking.type === 'warmup') updated.warmup = { name: item.name, duration: '8 min', desc: item.desc };
    else if (picking.type === 'station') updated.stations[picking.stationIdx] = { name: item.name, duration: '8 min', desc: item.desc };
    else if (picking.type === 'smallSided') updated.smallSided = { format: item.format || item.name, duration: '15 min', rules: item.desc };
    onUpdate(updated);
    setPicking(null);
  };
  ...
  return (
    <>
      <div onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 50, overflowY: 'auto', padding: 16, display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
        <div onClick={e => e.stopPropagation()}
          style={{ background: '#0f172a', borderRadius: 14, maxWidth: 660, width: '100%', border: `2px solid ${week.colour}`, overflow: 'hidden', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setEditMode(m => !m)}
                  style={{ padding: '6px 12px', border: 'none', borderRadius: 7, background: editMode ? '#fff' : 'rgba(255,255,255,0.2)', color: editMode ? week.colour : '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                  {editMode ? '✓ Done' : '✏️ Edit'}
                </button>
                <button onClick={onClose}
                  style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#fff', borderRadius: 7, width: 30, height: 30, fontSize: 14, cursor: 'pointer' }}>✕</button>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
              {['⏱ 60 min', '🏑 U10', '👥 ~20 players'].map(t => (
          ...
          <div style={{ padding: '18px 20px' }}>
            <SessionBlock label="🌡️ WARM-UP" duration="8 min" colour="#f59e0b" editMode={editMode} onSwap={() => setPicking({ type: 'warmup' })}>
            ...
            {week.stations.map((s, i) => (
              <SessionBlock key={i} label={`Station ${i + 1}`} duration="8 min" colour={week.colour} editMode={editMode} onSwap={() => setPicking({ type: 'station', stationIdx: i })}>
            ...
            <SessionBlock label="🏆 SMALL-SIDED GAME" duration="15 min" colour="#10b981" editMode={editMode} onSwap={() => setPicking({ type: 'smallSided' })}>
              <span style={{ background: '#064e3b', color: '#34d399', borderRadius: 5, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>{week.smallSided.format}</span>
            ...
            <button onClick={onClose}
              style={{ width: '100%', marginTop: 6, padding: 11, border: 'none', borderRadius: 9, background: week.colour, color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
              ← Back to Plan
            </button>
          </div>
        </div>
      ...
      {picking && (
        <DrillPicker type={picking.type} stationIdx={picking.stationIdx} accentColour={week.colour}
          onSelect={handlePickerSelect} onClose={() => setPicking(null)} />
      )}
    </>
  );
}
